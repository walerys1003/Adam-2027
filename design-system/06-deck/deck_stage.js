  ...
  class DeckStage extends HTMLElement {
    ...
    _render() {
      ...
      menu.addEventListener('click', e => {
        const act =
          e.target && e.target.getAttribute && e.target.getAttribute('data-act')
        if (!act) return
    ...
    _onMessage(e) {
      // Source filter: only the embedding parent frame is allowed to
      // toggle presenter/preview/rail modes. Standalone decks (no parent)
      // are effectively dormant for these signals. Defense-in-depth on
      // top of Tiffany's protocol — Tiffany trusts any source, we don't.
